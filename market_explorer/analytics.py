...
def compute_insights(df: pd.DataFrame) -> list[str]:
    """
    Generate simple business insights in natural language.
    """
    insights = []
    ...
    top = top_companies(df, n=5)
    share = top["Revenue_M"].sum(skipna=True) / total_rev if total_rev else 0

    insights.append(
        f"Top 5 companies represent {share:.0%} of total market revenue."
    )
